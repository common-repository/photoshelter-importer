...
use function PhotoShelter\Importer\REST_API\get_user_id;
...
function init() {
	...
	add_settings_field(
		...
		array(
			...
			'logout_text' => __( 'Log out', 'photoshelter-importer' ),
			'logout_url'  => get_plugin_settings_link( 'logout' ),
		)
	);
	...
	if ( false === empty( $api_key ) && false === empty( $client_id ) && true === empty( $access_token ) ) {
		add_settings_field(
	...
	if ( false === empty( $api_key ) && false === empty( $access_token ) ) {
		// Add Org ID field.
		add_settings_field(
	...
	if ( false === empty( $importer_token_set ) && true === isset( $importer_token_set['type'], $importer_token_set['message'] ) ) {
		add_settings_error(
		...
		function () {
			...
			switch ( $action ) {
				case 'logout':
					...
					update_option( 'photoshelter_importer_options', $options );
					update_option( 'photoshelter_importer_oauth_client_id', '' );
					update_option( 'photoshelter_importer_oauth_client_secret', '' );
					...
					update_option( 'photoshelter_importer_org_name', false );
					update_option( 'photoshelter_importer_user_id', '' );
					update_option( 'photoshelter_importer_user_logged_in', false );

					wp_safe_redirect( get_plugin_settings_link() );
...
function importer_settings_screen_section_account_field_api( $args ) {
	...
	$user_logged_in = (bool) get_option( 'photoshelter_importer_user_logged_in' );
...
function validate_importer_options( $input ) {
	...
	foreach ( $input as $key => $value ) {
		...
		if ( true === isset( $output['org_id'] ) ) {

			// Only send out an error if user entered something and it doesn't match the pattern.
			if (
				false === empty( $output['org_id'] ) &&
				false === preg_match( '#' . PHOTOSHELTER_API_REGEX_ORG_ID . '#', $output['org_id'] )
			) {
	...
	if ( false === isset( $errors['api_key'] ) && false === empty( $output['api_key'] ) ) {
		...
		if (
			true === isset( $output['org_id'] ) &&
			$output['org_id'] !== $previous_options['org_id'] &&
			false === isset( $errors['org_id'] )
		) {
...
function register_application( $api_key ) {
	...
	// Truncate application name to 80 characters (API limit).
	$application_name = substr( $application_name, 0, 80 );

	// Truncate name to 80 characters as well (API limit).
	$blog_name = substr( $blog_name, 0, 80 );
...
function process_organization( $org_id = '', $api_key = null ) {
	$api_key = true === is_null( $api_key ) ? get_api_key() : $api_key;

	if ( true === empty( $api_key ) ) {
...
/**
 * Get Plugin settings link string.
 *
 * @param string $action Optional action query arg string.
 *
 * @return string Settings link string.
 * @since 1.0.2
 */
function get_plugin_settings_link( $action = '' ) {
	$settings_link = admin_url(
		sprintf( 'options-general.php?page=%s', PHOTOSHELTER_IMPORTER_SLUG_SETTINGS )
	);

	if ( false === empty( $action ) ) {
		$settings_link = add_query_arg( compact( 'action' ), $settings_link );
	}
