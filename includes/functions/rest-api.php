...
use PhotoShelter\Importer\API\User\Client as User_Client;
use PhotoShelter\Importer\API\User\DataSource as User_DataSource;
...
function get_user_id() {
	return get_option( 'photoshelter_importer_user_id', '' );
}
...
function handle_importer_auth_callback( \WP_REST_Request $request ) {
	...
	if ( is_wp_error( $token_response ) ) {
		...
		update_option( 'photoshelter_importer_user_id', null );
		update_option( 'photoshelter_importer_user_logged_in', false );

		wp_safe_redirect( admin_settings_url( 'settings&authorized=false' ) );
	} else {
		...
		update_option( 'photoshelter_importer_user_logged_in', true );
	...
	$user         = new User_Client( new User_DataSource( get_api_key(), get_oauth_access_token() ) );
	$user_session = $user->get_session();

	if ( isset( $user_session->data->id ) ) {
		update_option( 'photoshelter_importer_user_id', $user_session->data->id );
	}
...
function handle_importer_token_callback( \WP_REST_Request $request ) {
	...
	if ( true === empty( $token ) || true === empty( $expires ) || true === empty( $refresh_token ) ) {
		return new \WP_Error(
			'bad-input',
...
function handle_importer_sideload( \WP_REST_Request $request ) {
	...
	if ( true === is_wp_error( $info ) ) {
		return error_response( $info, 400 );
	}

	if ( false === property_exists( $info->data->attributes, 'file_name' ) ) {
		return new \WP_Error(
			'api-fetch-error',
			sprintf(
	...
	$download = $media->download_media(
		compact( 'file_type', 'file_quality', 'media_id' )
	);
	...
	if ( false === $wp_filesystem->put_contents( $tmp_path, $download ) ) {
		return new \WP_Error(
			'file-error',
			sprintf(
				/* translators: %s is the tmp file path */
